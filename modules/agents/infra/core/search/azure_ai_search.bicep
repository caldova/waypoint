...
param knowledgeBaseName string = 'contracts-kb'

@description('Knowledge source name for the Foundry IQ blob knowledge source. The auto-generated indexer is derived as "<knowledgeSourceName>-indexer".')
param knowledgeSourceName string = 'contracts-ks'
...
// Search needs AI Services access for knowledge source embedding generation.
...
resource searchToFoundryUserRoleAssignment 'Microsoft.Authorization/roleAssignments@2022-04-01' = if (!empty(aiServicesAccountName)) {
  name: guid(aiServicesAccountName, searchService.id, 'Foundry User', uniqueString(deployment().name))
  scope: aiAccount
  properties: {
...
// User permissions - Search Service Contributor (needed for keyless index create/update)
...
// Foundry IQ MCP connection — lets the Foundry Toolbox call the KB MCP endpoint
// using the project's managed identity for auth against Azure AI Search.
...
output indexerName string = '${knowledgeSourceName}-indexer'
