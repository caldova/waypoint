...
- Deployment acceptance selects a live invoice from Waypoint, invokes the hosted
  ...
  `waypoint-recorder`.
- The Waypoint invoice queue exposes the same seller-operated, single-invoice
  trigger.
...
- FabricIQ remains optional until it is backed by a real Fabric Data Agent
  evidence source.
- Batch assurance and separate quality-operation triggers are not part of the
  validated seller flow.
