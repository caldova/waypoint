...
class AgentModelConfigurationTests(unittest.TestCase):
    def test_root_launch_deploys_only_foundryiq(self) -> None:
        ...
        self.assertEqual(manifest["deployment"]["evidence_plane"], "foundryiq")
        self.assertIn(
        ...
        self.assertNotIn("optional_agents", manifest["expected_components"])
        self.assertNotIn("fabric-provision:", workflow)
        self.assertNotIn("onelake-upload:", workflow)
        for removed_input in (
            "fabric_provision_enabled",
            "workiq_enabled",
            "webiq_enabled",
            "foundryiq_enabled",
            "fabriciq_enabled",
        ):
            self.assertNotIn(f"inputs.{removed_input}", workflow)
        self.assertIn(
    ...
    def test_app_deploy_retries_transient_registry_failures(self) -> None:
        ...
        self.assertIn("ASPIRE_DEPLOY_ATTEMPT_TIMEOUT:-20m", aspire_deploy)
        self.assertIn("for check in $(seq 1 20)", aspire_deploy)
        self.assertIn("timeout-minutes: 45", deploy_app)
    ...
    def test_clean_deploy_handles_resource_creation_races_and_soft_delete(self) -> None:
        ...
        self.assertLess(
            msal.index("ensuring teardown ownership tags"),
            msal.index("setting identifier uri"),
        )
    ...
    def test_acceptance_respects_launch_stage_upper_bounds(self) -> None:
        workflow = (ROOT / ".github/workflows/deploy.yml").read_text()
        acceptance = workflow.split("\n  acceptance:\n", 1)[1].split(
        ...
        self.assertIn('os.environ["DEPLOY_AGENTS_ENABLED"] == "true"', acceptance)
        self.assertNotIn("FABRIC_PROVISION_ENABLED", acceptance)
        self.assertNotIn("FOUNDRYIQ_ENABLED", acceptance)
        self.assertIn(
            "needs['provision-agents'].result == 'success'",
            acceptance,
        )
        self.assertIn(
            "needs['deploy-agents'].result == 'success'",
            acceptance,
        )
        self.assertIn(
            "needs['contracts-kb-upload'].result == 'success'",
            acceptance,
        )
        self.assertIn(
            "needs['seed-import'].result == 'success'",
            acceptance,
        )
